...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  /// Debounce sensor service restart to avoid too frequent restarts when adjusting settings
  void _debounceRestartMonitoring() {
  ...
  int _accelerometerSamplingRate = 20; // Hz
  int _gyroscopeSamplingRate = 10; // Hz
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _impactThreshold = _prefs.getDouble('impact_threshold') ?? 15.0;
      ...
      // Load sensor sampling rates
      _accelerometerSamplingRate =
          _prefs.getInt('accelerometer_sampling_rate') ??
          SensorConstants.accelerometerSensitivity;
      _gyroscopeSamplingRate =
          _prefs.getInt('gyroscope_sampling_rate') ??
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // Sensor Settings
          _buildSectionHeader('Sensor Settings'),
          _buildSliderTile(
            'Accelerometer Sampling Rate (Hz)',
            'How often to read accelerometer data (higher = more sensitive but uses more battery)',
            _accelerometerSamplingRate.toDouble(),
            5.0,
            50.0,
            Icons.sensors,
            (value) {
              setState(() => _accelerometerSamplingRate = value.round());
              _saveSetting('accelerometer_sampling_rate', value.round());
              // Will restart monitoring after a short delay to avoid too frequent restarts
              _debounceRestartMonitoring();
            },
          ),
          _buildSliderTile(
            'Gyroscope Sampling Rate (Hz)',
            'How often to read gyroscope data (higher = more sensitive but uses more battery)',
            _gyroscopeSamplingRate.toDouble(),
            1.0,
            25.0,
            Icons.screen_rotation,
            (value) {
              setState(() => _gyroscopeSamplingRate = value.round());
              _saveSetting('gyroscope_sampling_rate', value.round());
              // Will restart monitoring after a short delay to avoid too frequent restarts
              _debounceRestartMonitoring();
            },
          ),
  ...
  Future<void> _resetSettings() async {
    ...
    // Also restart sensor service with default settings
    _debounceRestartMonitoring();
